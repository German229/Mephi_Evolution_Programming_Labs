...
*Рис. 4. Четыре характерных решения (слева направо: минимум воды, колено, малый стресс, без стресса). Сверху — запас влаги $\theta$: среднее по сценариям и коридор min…max по 10 сценариям; красная зона — стресс ($\theta<50$), синяя — переувлажнение ($\theta>100$). Снизу — поливные нормы и средние осадки по дням.*
...
|---|---|---|---|---|---|
| **минимум воды** («дешёвый») | самое дешёвое решение фронта | 56.0 | 301.9 | 2.2 | 19 |
| **сбалансированный** (колено) | ближайшая к идеальной точке после нормировки на размах фронта | 101.9 | 137.4 | 4.5 | 24 |
| **малый стресс** | минимум воды при $f_2 \le 5\,\%$ от дефицита без полива (≤ 26.9) | 138.9 | 26.7 | 13.8 | 28 |
| **без стресса** («надёжный») | $f_2 = 0$, минимум среди таких | 158.6 | 0.0 | 23.3 | 28 |

Смысл:
- **Минимум воды** — «дешёвый» край фронта: 56 мм воды, почти нет переувлажнения, но сильный стресс (302 мм·сут, ≈56 % от дефицита без полива). Это поддерживающий полив, чтобы культура только выжила: при жёстком лимите воды или для засухоустойчивых культур вне критических фаз.
- **Без стресса** — для культур, чувствительных к недостатку влаги (овощи, фаза цветения), и дешёвой воды. Ни в одном из 10 сценариев $\theta$ не опускается ниже порога стресса (рис. 4, справа). Плата — 159 мм воды и переувлажнение 23 мм·сут: в сценариях с ливнем запас долго держится выше $FC$, вода уходит в дренаж, растёт риск вымывания удобрений и гипоксии корней.
- **Малый стресс** — практический компромисс: на 20 мм (12 %) меньше воды, чем «без стресса», при остаточном дефиците около 5 % от дефицита без полива. В сухих сценариях $\theta$ лишь кратковременно касается зоны стресса, переувлажнение на 40 % ниже. Подходит при умеренной цене воды и засухоустойчивой культуре.
- **Сбалансированный (колено)** — режим при дорогой или лимитированной воде: 102 мм, почти нет переувлажнения (4.5), но заметный стресс (137 мм·сут, ≈26 % от дефицита без полива). В сухих сценариях $\theta$ опускается до 20–30 мм (рис. 4). Большой стресс у «колена» — свойство выбранного правила, а не ошибка. Нормировка на размах фронта уравнивает оси, а размах дефицита (0…302) велик по сравнению с размахом переувлажнения (2…23). Поэтому «геометрически середина» оказывается в зоне заметного стресса. Для конкретной культуры агроном мог бы задать другие масштабы.
...
- Характерные решения физически осмысленны и согласуются с агрономической интуицией: безопасный режим требует ≈160 мм при потребности культуры ≈178 мм и средних осадках 85 мм (запас воды нужен против сухих сценариев), режим «малый стресс» — на 12 % меньше.
...
13. **Что изменится при росте размерности?** Горизонт 60–90 суток или несколько полей увеличат число переменных пропорционально. Гиперобъём по-прежнему считается точно (для 3 критериев — $O(n^2\log n)$), но при 4 и более критериях точный HV и crowding distance становятся неэффективными, и лучше перейти на NSGA-III или оценку HV методом Монте-Карло. Эволюция покоординатного расписания масштабируется плохо, поэтому при длинном горизонте разумнее эволюционировать параметры правила полива.
14. **Что даст больший бюджет?** Для $\eta_m = 20$ гиперобъём к концу бюджета ещё растёт, так что прирост будет. Для $\eta_m = 5$ фронт близок к сходимости. Полезнее другое: больше сценариев осадков (точнее оценка $f_2$ и $f_3$) и внешний архив недоминируемых решений.
15. **Что экспериментально, а что гарантировано?** Гарантированы: недоминируемость итогового набора (проверяется в тесте), корректность сортировки (сверена с полным перебором), точность гиперобъёма (аналитика и Монте-Карло), баланс массы воды в модели и воспроизводимость по seed. Экспериментальны: что фронт близок к истинному фронту Парето (он неизвестен), значения HV и превосходство над свёрткой — они верны для этой модели, 10 сценариев и бюджета 20 000.
