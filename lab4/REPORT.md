...
Заявка (обязательное требование 1 ЛР4) — [`PROPOSAL.md`](PROPOSAL.md). Она была утверждена до начала реализации и содержит постановку, представление, алгоритм, данные, критерии оценки и риски. Реализация следует заявке: тот же язык функций, две задачи (Nguyen-7, Pagie-1), шум 5 %, разбиение 60/20/20, четыре механизма A–D, 30 запусков, U-тест с поправкой Холма и $\hat A_{12}$. Отличия следующие.
...
- дополнительно: бинарный турнир D даёт меньшее селективное давление по ошибке, чем $k=7$ у A–C.
...
1. Контроль разрастания необходим: без него (A) размер моделей в 6–20 раз больше, а вычисления в 2.7–28 раз дороже **без выигрыша в точности** на test ни на одной задаче.
...
4. **NSGA-II по (ошибка, размер) без мер поддержки разнообразия непригоден для трудной задачи**: давление на размер схлопывает популяцию к малым деревьям, ошибка значимо хуже ($p\approx10^{-7}$). На простой задаче он даёт самую компактную «лестницу» формул.
