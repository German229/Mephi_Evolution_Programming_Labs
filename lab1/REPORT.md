...
Ранговая селекция (`rank_s`) и равномерный кроссовер реализованы в ядре, но в этих конфигурациях не используются.
...
$U = 0$ означает полное разделение: каждый запуск A лучше каждого запуска B. Для пары «адапт. σ» и «адапт. σ, $p_m=0.3$» тест по `*_summary.csv` даёт $U \approx 150.5$ из 400, $z \approx -1.3$, $p \approx 0.2$. Различие медиан (0.025 и 0.0086) статистически не подтверждено.

### 7.3. Графики

![Сходимость базового ГА](results/conv_ga_base.png)

*Рис. 1. Эксперимент 2: best-so-far базового ГА по поколениям, 20 запусков; медиана, IQR и диапазон min–max, логарифмическая шкала. За ~20 поколений $f$ падает со $\sim10^2$ до $\sim1$, затем медленно снижается до медианы 0.18. Разброс узкий: 0.07–0.27.*
...
- **Самоадаптивный σ снимает это противоречие.** Только он дал глобальный минимум (до 30 % запусков), улучшения значимы ($p < 10^{-6}$).
...
- **Ограничения эксперимента.** 20 запусков мало, чтобы уловить тонкие различия (как в случае с $p_m$). База σ = 0.01 — не лучшее фиксированное значение: σ = 0.001 дало медиану 0.013 на seed 0–9, что не хуже адаптивного варианта по медиане. Поэтому преимущество самоадаптации над *наилучшим* фиксированным σ проявляется в доле успехов, а не в медиане. Наконец, `ga_adaptive_pm03` отличается от базы двумя факторами.
...
# все эксперименты ЛР1 (5 серий × 20 запусков), ~5 с
